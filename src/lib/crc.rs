...
fn calculate_crc8(data: &[u8]) -> u8 {
    let mut crc = 0u8; // Initial value
    for &byte in data {
        crc = CRC8X_TABLE[(byte ^ crc) as usize];
    }
    crc
}
...
pub fn receive_data(
    ...
    match state {
        SerialState::StartByte(_) => {
            let mut temp_buf: [u8; 1] = [0];
            if let Err(err) = port.read_exact(&mut temp_buf) {
                eprintln!("[ERROR]: {err}");
        ...
        SerialState::Working => {
            if let Err(err) = port.read_exact(buf) {
                eprintln!("[ERROR]: {err}");
                return SerialState::StartByte(None);
            } else {
                let calculated_crc = calculate_crc8(&buf[..buf.len() - 1]);
                if calculated_crc == *buf.last().unwrap() {
